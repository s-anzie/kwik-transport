...
func TestChunkManager_StartTransfer(t *testing.T) {
	...
	metadata := types.NewFileMetadata("test.txt", 1024, 256)
...
func TestChunkManager_ReceiveChunk(t *testing.T) {
	...
	// Start transfer
	metadata := types.NewFileMetadata("test.txt", 512, 256) // 2 chunks
	...
	chunk1 := types.NewFileChunk(1, 0, data1, "test.txt", 0, 2, false)
	...
	chunk2 := types.NewFileChunk(2, 1, data2, "test.txt", 256, 2, true)
...
func TestChunkManager_ReceiveChunk_InvalidChecksum(t *testing.T) {
	...
	metadata := types.NewFileMetadata("test.txt", 256, 256)
	...
	data := []byte("test data")
	chunk := types.NewFileChunk(1, 0, data, "test.txt", 0, 1, true)
...
func TestChunkManager_GetMissingChunks(t *testing.T) {
	...
	metadata := types.NewFileMetadata("test.txt", 1024, 256) // 4 chunks
	err = cm.StartTransfer(metadata, nil)
	if err != nil {
		t.Fatalf("Failed to start transfer: %v", err)
	}
	...
	// Receive chunk 1
	data := make([]byte, 256)
	chunk := types.NewFileChunk(1, 1, data, "test.txt", 256, 4, false)
	err = cm.ReceiveChunk(chunk)
	if err != nil {
...
func TestChunkManager_GetTransferProgress(t *testing.T) {
	...
	// Receive a chunk
	data := make([]byte, 256)
	chunk := types.NewFileChunk(1, 0, data, "test.txt", 0, 4, false)
	err = cm.ReceiveChunk(chunk)
	if err != nil {
		t.Fatalf("Failed to receive chunk: %v", err)
	}
...
func TestChunkManager_FileReconstruction(t *testing.T) {
	...
	chunkSize := int32(20)
	metadata := types.NewFileMetadata("test.txt", int64(len(originalData)), chunkSize)
	...
	for _, i := range chunkOrder {
		...
		chunk := types.NewFileChunk(uint32(i+1), uint32(i), chunkData, "test.txt", 
...
func TestChunkManager_GetAllTimedOutChunks(t *testing.T) {
	...
	// Start two transfers
	metadata1 := types.NewFileMetadata("test1.txt", 512, 256) // 2 chunks
	err = cm.StartTransfer(metadata1, nil)
	if err != nil {
		t.Fatalf("Failed to start transfer 1: %v", err)
	}

	metadata2 := types.NewFileMetadata("test2.txt", 768, 256) // 3 chunks
...
func TestChunkManager_ShouldRetryChunk(t *testing.T) {
	...
	metadata := types.NewFileMetadata("test.txt", 256, 256) // 1 chunk
	err = cm.StartTransfer(metadata, nil)
	if err != nil {
		t.Fatalf("Failed to start transfer: %v", err)
	}
...
func TestChunkManager_ReceiveChunkWithValidation_InvalidSize(t *testing.T) {
	...
	metadata := types.NewFileMetadata("test.txt", 256, 256) // 1 chunk of 256 bytes
	err = cm.StartTransfer(metadata, nil)
	if err != nil {
		t.Fatalf("Failed to start transfer: %v", err)
	}
...
func TestChunkManager_GetIntegrityReport(t *testing.T) {
	...
	// Create chunk with wrong size to trigger validation error
	data := make([]byte, 128) // Wrong size
	chunk := types.NewFileChunk(1, 0, data, "test.txt", 0, 2, false)
