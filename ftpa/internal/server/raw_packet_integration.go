...
// SecondaryServer interface defines the methods needed by RawPacketHandler
type SecondaryServer interface {
	readChunkFromFile(command *types.ChunkCommand) (*types.FileChunk, error)
	sendChunkToClient(chunk *types.FileChunk, command *types.ChunkCommand) error
	notifyPrimaryServer(command *types.ChunkCommand, success bool, errorMessage string)
	cleanupCommand(commandID string)
}

// RawPacketHandler handles raw packet transmissions for the secondary file server
...
// NewRawPacketHandler creates a new raw packet handler for the secondary file server
func NewRawPacketHandler(secondaryServer SecondaryServer, kwikSession session.Session) *RawPacketHandler {
...
func (rph *RawPacketHandler) handleRawPacketStream(stream session.Stream) {
	...
	var chunkCommand types.ChunkCommand
...
func (rph *RawPacketHandler) handleDirectChunkCommand(command *types.ChunkCommand, stream session.Stream) {
	// Validate the chunk command
...
func (rph *RawPacketHandler) processChunkCommandFromRawPacket(command *types.ChunkCommand, rawPacketMsg *RawPacketMessage) {
	switch command.Command {
	case "SEND_CHUNK":
...
func (rph *RawPacketHandler) processRawPacketSendChunkCommand(command *types.ChunkCommand, rawPacketMsg *RawPacketMessage) {
	...
	rph.commandsMutex.Lock()
	rph.activeCommands[commandID] = commandState
	rph.commandsMutex.Unlock()
...
func (rph *RawPacketHandler) processDirectSendChunkCommand(command *types.ChunkCommand, stream session.Stream) {
	// Create command ID for tracking
	commandID := fmt.Sprintf("%s_%d_%d", command.Filename, command.ChunkID, command.SequenceNum)
...
func (rph *RawPacketHandler) processRawPacketChunkCommand(commandID string, commandState *CommandState, rawPacketMsg *RawPacketMessage) {
	...
	chunk, err := rph.secondaryServer.readChunkFromFile(command)
	if err != nil {
	...
	err = rph.secondaryServer.sendChunkToClient(chunk, command)
	if err != nil {
	...
	go func() {
		...
		rph.secondaryServer.cleanupCommand(commandID)
	}()
}
...
func (rph *RawPacketHandler) processDirectChunkCommand(commandID string, commandState *CommandState) {
	...
	rph.secondaryServer.notifyPrimaryServer(command, true, "")
...
func (rph *RawPacketHandler) sendRawPacketAckResponse(command *types.ChunkCommand, rawPacketMsg *RawPacketMessage) {
...
func (rph *RawPacketHandler) sendRawPacketSuccessResponse(command *types.ChunkCommand, rawPacketMsg *RawPacketMessage) {
...
func (rph *RawPacketHandler) sendRawPacketErrorResponse(command *types.ChunkCommand, rawPacketMsg *RawPacketMessage, errorMessage string) {
...
func (rph *RawPacketHandler) sendDirectAckResponse(stream session.Stream, command *types.ChunkCommand) error {
	response := map[string]interface{}{
		"type":         "COMMAND_ACK",
		"command":      command.Command,
		"chunk_id":     command.ChunkID,
...
func (rph *RawPacketHandler) handleCommandError(commandID string, err error) {
	rph.commandsMutex.Lock()
	defer rph.commandsMutex.Unlock()

	commandState, exists := rph.activeCommands[commandID]
	if !exists {
		return
	}
