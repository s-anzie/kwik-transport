...
func NewStandaloneFileTransferServer(serverConfig *config.ServerConfiguration) (*StandaloneFileTransferServer, error) {
	if serverConfig == nil {
	...
	server := &StandaloneFileTransferServer{
		address:       serverConfig.Server.Address,
		config:        serverConfig,
...
func (s *StandaloneFileTransferServer) handleClient(clientID string, clientSession session.Session) {
	...
	if s.config.Server.SecondaryAddress != "" {
	...
	coordinator, err := NewChunkCoordinator(
		clientSession,
		s.config.Performance.SecondaryPaths,
		s.config.Server.FileDirectory,
		s.config.Performance.ChunkSize,
		s.config.Limits.MaxConcurrent,
	)
	if err != nil {
	...
	fileServer, err := NewFileTransferServer(s.config)
	if err != nil {
	...
	// Assign the client session to the file server for this client
	fileServer.session = clientSession
...
func (s *StandaloneFileTransferServer) setupMultiPath(clientSession session.Session, clientID string) {
	...
	if serverSession, ok := clientSession.(*session.ServerSession); ok {
		pathID := serverSession.GetPendingPathID(s.config.Server.SecondaryAddress)
		if pathID != "" {
			s.config.Performance.SecondaryPaths = []string{pathID}
