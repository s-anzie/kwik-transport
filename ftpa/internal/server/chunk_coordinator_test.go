...
// Helper function to create a mock session with specified paths
func createMockSessionWithPaths(paths []session.PathInfo) *utils.MockSession {
	mockSession := utils.NewMockSession()
	mockSession.SetActivePaths(paths)
	return mockSession
}

// Helper function to create standard two-path mock session
func createStandardMockSession() *utils.MockSession {
	return createMockSessionWithPaths([]session.PathInfo{
		{PathID: "primary", IsPrimary: true, Status: session.PathStatusActive},
		{PathID: "secondary1", IsPrimary: false, Status: session.PathStatusActive},
	})
}
...
func TestChunkCoordinatorSecondaryCoordination(t *testing.T) {
	...
	mockSession := utils.NewMockSession()
	mockSession.SetActivePaths([]session.PathInfo{
		{PathID: "primary", IsPrimary: true, Status: session.PathStatusActive},
		{PathID: "secondary1", IsPrimary: false, Status: session.PathStatusActive},
	})
...
func TestSecondaryServerResponseHandling(t *testing.T) {
	...
	successResponse := types.ChunkCommand{
		Command:     "CHUNK_SENT",
...
func TestSecondaryServerErrorHandling(t *testing.T) {
	...
	errorResponse := types.ChunkCommand{
		Command:     "CHUNK_ERROR",
		ChunkID:     secondaryChunk,
		SequenceNum: secondaryChunk,
		Filename:    "test.txt",
...
func TestGetSecondaryServerStatus(t *testing.T) {
	...
	mockSession.SetActivePaths([]session.PathInfo{
		{PathID: "primary", IsPrimary: true, Status: session.PathStatusActive},
		{PathID: "secondary1", IsPrimary: false, Status: session.PathStatusActive},
		{PathID: "secondary2", IsPrimary: false, Status: session.PathStatusActive},
	})
...
func TestReassignFailedChunks(t *testing.T) {
	...
	// Create mock session
	mockSession := createStandardMockSession()
...
func TestBandwidthBasedDistribution(t *testing.T) {
	...
	metadata := types.NewFileMetadata("large_test.txt", 1000, 100)
...
func TestDynamicRebalancing(t *testing.T) {
	...
	metadata := types.NewFileMetadata("test.txt", 1000, 100)
	transferState := &TransferState{
		Filename:         "test.txt",
		Metadata:         metadata,
...
func TestOptimizationConfiguration(t *testing.T) {
	...
	// Create mock session
	mockSession := createMockSessionWithPaths([]session.PathInfo{
		{PathID: "primary", IsPrimary: true, Status: session.PathStatusActive},
	})
