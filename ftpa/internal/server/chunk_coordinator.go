...
type TransferState struct {
	...
	stream           session.Stream             // KWIK stream for this transfer
}
...
func (cc *ChunkCoordinator) StartFileTransfer(stream session.Stream, filename string, clientAddress string) (*TransferState, error) {
...
func (cc *ChunkCoordinator) sendChunk(transferState *TransferState, sequenceNum uint32, serverType ServerType) error {
	...
	err = cc.sendChunkViaStream(transferState.stream, chunk)
...
func (cc *ChunkCoordinator) sendChunkCommand(transferState *TransferState, sequenceNum uint32) error {
	...
	err = cc.primarySession.SendRawData(commandData, pathID, transferState.stream.RemoteStreamID())
	if err != nil {
...
func (cc *ChunkCoordinator) sendChunkViaStream(stream session.Stream, chunk *types.FileChunk) error {
	// Serialize chunk
