...
type FileTransferServerConfig struct {
	...
	SecondaryAddress  string   // Address of secondary server for multi-path
}

// NewFileTransferServer creates a new file transfer server
func NewFileTransferServer(kwikSession session.Session, config *FileTransferServerConfig) (*FileTransferServer, error) {
	// Validate parameters
	if kwikSession == nil {
...
func (fts *FileTransferServer) sendMetadataResponse(stream session.Stream, metadata *FileMetadata) error {
