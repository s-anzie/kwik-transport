...
func NewSecondaryFileHandler(kwikSession session.Session, config *SecondaryFileHandlerConfig) (*SecondaryFileHandler, error) {
	// Validate parameters
	if kwikSession == nil {
	...
	handler.rawPacketHandler = NewRawPacketHandler(handler, kwikSession)
...
func (sfh *SecondaryFileHandler) handleCommandStream(stream session.Stream) {
	defer stream.Close()
...
func (sfh *SecondaryFileHandler) handleSendChunkCommand(stream session.Stream, command *ChunkCommand) {
...
func (sfh *SecondaryFileHandler) sendCommandAck(stream session.Stream, command *ChunkCommand) error {
...
func (sfh *SecondaryFileHandler) sendErrorResponse(stream session.Stream, errorMessage string) {
	response := map[string]interface{}{
		"type":      "ERROR",
		"error":     errorMessage,
		"server_id": sfh.serverID,
