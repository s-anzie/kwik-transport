...
type ChunkTransferState struct {
	...
	ReceivedChunks   map[uint32]*FileChunk // Chunks received so far (by sequence number)
...
func (cm *ChunkManager) StartTransfer(metadata *FileMetadata, progressCallback func(progress float64)) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
	...
	state := &ChunkTransferState{
		Metadata:         metadata,
		ReceivedChunks:   make(map[uint32]*FileChunk),
...
func (cm *ChunkManager) ReceiveChunk(chunk *FileChunk) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
...
// ChunkRetryRequest represents a request to retransmit a missing chunk
type ChunkRetryRequest struct {
	Filename    string    `json:"filename"`
	ChunkID     uint32    `json:"chunk_id"`
	SequenceNum uint32    `json:"sequence_num"`
	RetryCount  int       `json:"retry_count"`
	LastRequest time.Time `json:"last_request"`
	Priority    int       `json:"priority"` // Higher priority for chunks that have been missing longer
}
...
func (cm *ChunkManager) GetTimedOutChunks(filename string) ([]ChunkRetryRequest, error) {
	cm.mu.RLock()
	defer cm.mu.RUnlock()
	...
	var timedOutChunks []ChunkRetryRequest
	now := time.Now()
	...
	for sequenceNum := range state.MissingChunks {
		...
		if !hasTimestamp || now.Sub(lastRequest) > cm.chunkTimeout {
			...
			timedOutChunks = append(timedOutChunks, ChunkRetryRequest{
				Filename:    filename,
...
func (cm *ChunkManager) GetAllTimedOutChunks() ([]ChunkRetryRequest, error) {
	cm.mu.RLock()
	...
	var allTimedOut []ChunkRetryRequest
...
func (cm *ChunkManager) ReceiveChunkWithValidation(chunk *FileChunk, maxCorruptionRetries int) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
