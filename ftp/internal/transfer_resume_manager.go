...
func (trm *TransferResumeManager) StartTransfer(metadata *FileMetadata, transferID string) (*TransferResumeState, bool, error) {
	trm.mu.Lock()
	defer trm.mu.Unlock()
...
func (trm *TransferResumeManager) canResumeTransfer(state *TransferResumeState, metadata *FileMetadata) bool {
...
func (trm *TransferResumeManager) RecordChunkReceived(filename string, chunk *FileChunk) error {
	trm.mu.Lock()
	defer trm.mu.Unlock()
